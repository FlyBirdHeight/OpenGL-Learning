...
#include "perlin_old.hpp"
#include <fstream>
PerlinNoiseOld::PerlinNoiseOld(){
    this->generateMesh();
}


//创建晶格并生成梯度向量
void PerlinNoiseOld::generateMesh(){
    unsigned seed = 2016;
...
//生成对应的晶格数据
float PerlinNoiseOld::generateMeshData(glm::fvec2 point){
    //左下角晶格顶点
...
//梯度向量获取
glm::fvec2 PerlinNoiseOld::getIndex(int x, int y){
    return this->gradient[(x + this->permutationTable[y]) % 256];
}
//缓和曲线
float PerlinNoiseOld::smoothCurve(float data){
    float returnData = data * data * (3.0 - 2.0 * data);
    
    return returnData;
}
//fbm
float PerlinNoiseOld::getNoiseByFbm(int octaves, float x, float y){
    double output = .0;
    double denom = .0;
    double frequency = 1.;
    double amplitude = 1.;
...
//测试
void PerlinNoiseOld::testHash(int x, int y){
    this->generateMesh();
