...
//        m_pointDistance = pow(m_pointMass / m_restDensity , 1.0f / 3.0f);
//        this->addFluidVolume(initFluidBox, m_pointDistance / m_unitScale);
...
//        float h_2 = m_smoothRadius * m_smoothRadius;
...
//                        float pv_2 = pv.x * pv.x + pv.y * pv.y + pv.z * pv.z;
//                        if(pv_2 < h_2){
...
//            Point *p = m_pointType.get(i);
//            glm::fvec3 accel_sum = glm::fvec3(0.0f, 0.0f, 0.0f);
//            int neighCount = m_neighborTable.getNeighborCounts(i);
//            if(frameCount == 0){
//                std::cout << neighCount << std::endl;
//            }
...
//                //计算压强下的作用力
//                float pressure = (-1 * m_pointMass) * m_kernelSpiky * h_r * h_r * (p->pressure + pN->pressure) / (2 * (p->density * pN->density));
//                accel_sum += (ri_rj * pressure) / r ;
//                //计算粘性力
//                float viscosity = (m_kernelViscosity * m_viscosity * h_r * m_pointMass )/ (p->density * pN->density);
//                accel_sum += (pN->velocity_eval - p->velocity_eval) * viscosity;
//            }
//            p->accel = accel_sum;
//        }
//    }
...
//        float sl_2 = m_speedLimiting * m_speedLimiting;
//        float borderLineWeigh = 1;
//        for(unsigned int i = 0; i < m_pointType.size(); ++i){
...
//                accel *= m_speedLimiting / std::sqrt(accelLength_2);
//            }
//            float diff;
//           //边界情况
//           // Z方向边界
//           diff = borderLineWeigh * m_unitScale - (point->position.z - m_sphWallBox.min.z)*m_unitScale;
//           if (diff > 0.0f )
//           {
//               glm::fvec3 norm(0, 0, 1.0);
...
//           }
//
//           diff = borderLineWeigh * m_unitScale - (m_sphWallBox.max.z - point->position.z)*m_unitScale;
//           if (diff > 0.0f)
//           {
//               glm::fvec3 norm( 0, 0, -1.0);
//               float adj = m_boundartStiffness * diff - m_boundaryDampening *glm::dot ( norm,point->velocity_eval );
...
//
//           //X方向边界
//           diff = borderLineWeigh * m_unitScale - (point->position.x - m_sphWallBox.min.x)*m_unitScale;
//           if (diff > 0.0f )
//           {
//               glm::fvec3 norm(1.0, 0, 0);
//               float adj = m_boundartStiffness * diff - m_boundaryDampening * glm::dot ( norm,point->velocity_eval ) ;
//               accel.x += adj * norm.x;
//               accel.y += adj * norm.y;
//               accel.z += adj * norm.z;
//           }
//
//           diff = borderLineWeigh * m_unitScale - (m_sphWallBox.max.x - point->position.x)*m_unitScale;
//           if (diff > 0.0f)
//           {
//               glm::fvec3 norm(-1.0, 0, 0);
//               float adj = m_boundartStiffness * diff - m_boundaryDampening * glm::dot ( norm,point->velocity_eval );
...
//
//           //Y方向边界
//           diff = borderLineWeigh * m_unitScale - ( point->position.y - m_sphWallBox.min.y )*m_unitScale;
//           if (diff > 0.0f)
//           {
//               glm::fvec3 norm(0, 1.0, 0);
//               float adj = m_boundartStiffness * diff - m_boundaryDampening * glm::dot ( norm,point->velocity_eval );
//               accel.x += adj * norm.x;
//               accel.y += adj * norm.y;
//               accel.z += adj * norm.z;
//           }
//           diff = borderLineWeigh * m_unitScale - ( m_sphWallBox.max.y - point->position.y )*m_unitScale;
//           if (diff > 0.0f)
//           {
//               glm::fvec3 norm(0, -1.0, 0);
//               float adj = m_boundartStiffness * diff - m_boundaryDampening * glm::dot ( norm,point->velocity_eval );
