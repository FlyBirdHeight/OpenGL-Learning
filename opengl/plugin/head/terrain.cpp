...
void Terrain::generate_map_chunk(unsigned int &mapChunkVao, std::vector<float> vertices, std::vector<int> indices, std::vector<float> colorCard, std::vector<float> normals){
    ...
    int indicesData[indices.size()];
    float verticesData[vertices.size()];
    float colorCardData[colorCard.size()];
    float normalsData[normals.size()];
    ...
    for(int i = 0; i < colorCard.size(); ++i){
        colorCardData[i] = colorCard[i];
    }
    for(int i = 0; i < normals.size(); ++i){
        normalsData[i] = normals[i];
    }
    unsigned int EBO;
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(colorCardData), colorCardData, GL_STATIC_DRAW);
